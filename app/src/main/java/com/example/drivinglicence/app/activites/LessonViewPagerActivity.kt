...
class LessonViewPagerActivity : BaseVMActivity<ActivityLessonViewPagerBinding, MapDataViewModel>() {
    ...
    override fun initListener() {
        ...
        binding.viewPager.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                // Cập nhật giao diện nút Back/Forward và số thứ tự câu hỏi
                updateControls(position)

                // =====================================================================
                // == GỌI HÀM LƯU TIẾN ĐỘ KHI NGƯỜI DÙNG CHUYỂN TRANG (Rất quan trọng) ==
                // =====================================================================
                if (listQuestion.isNotEmpty() && position < listQuestion.size) {
                    val currentQuestion = listQuestion[position]
                    onQuestionViewed(currentQuestion.questionId)
                }
            }
    ...
    // Biến lưu trữ danh sách câu hỏi và câu trả lời cho màn hình hiện tại
    ...
    /**
     * Hàm này lưu ID của câu hỏi đã được xem vào MMKV.
     * Sử dụng Set để đảm bảo không có ID nào bị lưu trùng lặp.
     */
    private fun onQuestionViewed(questionId: Int) {
        val mmkv = MMKV.defaultMMKV()
        // Lấy danh sách (Set) các ID đã xem
        val viewedQuestionsSet = mmkv.decodeStringSet("VIEWED_QUESTIONS_SET", mutableSetOf()) ?: mutableSetOf()
        // Thêm ID mới vào và lưu lại
        viewedQuestionsSet.add(questionId.toString())
        mmkv.encode("VIEWED_QUESTIONS_SET", viewedQuestionsSet)
    }
    ...
    /**
     * Tách logic cập nhật UI ra một hàm riêng để tái sử dụng.
     */
    @SuppressLint("SetTextI18n")
